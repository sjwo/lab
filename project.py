...
NODE = platform.node()
# UNH AI compute and spare clusters (currently, `platform.node()` returns just the hostname, e.g. `ai0`, not the fully-qualified network name, e.g. `ai0.cs.unh.edu`.
REMOTE = re.match(r"ais?[0-9]+", NODE)
